 ...
 * @version $Id: ups.php 14498 2009-10-01 20:16:16Z ajeh $
 */
/**
...
$chk_ups_cost = (($cost * $shipping_num_boxes) + ($type == 'GND' ? 0.00 : (MODULE_SHIPPING_UPS_HANDLING_METHOD == 'Box' ? MODULE_SHIPPING_UPS_HANDLING * $shipping_num_boxes : MODULE_SHIPPING_UPS_HANDLING)));
//echo 'UPS C2A1 $chk_ups_cost: ' . $chk_ups_cost . ($chk_ups_cost <= 49 ? ' YES LOWER!' : ' NO HIGHER') . ($_SESSION['ups_low'] <= 49 ? ' ups_low YES LOWER: ' : ' ups_low NO HIGHER: ') . ' $_SESSION[ups_low]: ' . $_SESSION['ups_low'] . ' $type: ' . $type . ' $cost: ' . $cost . '<br>';
if ($chk_ups_cost <= 49.00 || !isset($_SESSION['ups_low'])) {
  $_SESSION['ups_low'] = $chk_ups_cost;
//echo 'UPS C2A2 $chk_ups_cost: ' . $chk_ups_cost . ($chk_ups_cost <= 49 ? ' YES LOWER!' : ' NO HIGHER') . ($_SESSION['ups_low'] <= 49 ? ' ups_low YES LOWER: ' : ' ups_low NO HIGHER: ') . ' $_SESSION[ups_low]: ' . $_SESSION['ups_low'] . '<br>';
}
